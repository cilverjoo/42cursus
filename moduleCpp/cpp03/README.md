...
CMiddleA::m_Top = 10;
```
다음과 같이 범위연산자를 사용한다면 m_Top을 각자 사용할 수 있게 되지만, 하나의 m_Top만을 필요로 하기 때문에 사실상 두 번째 m_Top은 메모리 낭비만 일으키게 된다. 그래서 다중 상속의 다이아몬드 구조에 의한 메모리 낭비를 줄이기 위한 특별한 구조가 제시되는데 그게 **가상 상속**이다.


#### 2. 가상 상속이란?

- 다중 상속은 **멤버의 중복**에 의한 메모리 낭비를 가져올 수 있다. 다중 상속으로 인한 모호성 문제와 메모리 낭비를 일거양득으로 해결할 수 있는 방법이 바로 **가상 상속**이다. 가상 상속은 실제 프로젝트에서 그리 많이 사용되지 않는다고 한다.
