...
### heap
* heap은 동적 메모리 할당에 사용되는 메모리를 추적 c++에서는 new 연산자를 사용해서 메모리를 할당하면 이 메모리는 heap에 할당 동적으로 할당된 변수가 삭제되면 메모리는 heap으로 반환되고, 재할당 가능 포인터를 삭제하면 변수가 삭제되는 것이 아니라 관련 변수의 메모리를 운영체제에 반환
...
* 깊은 복사의 경우에는 새로이 동적 할당은 받고, 원본의 데이터까지 복사를 한다.
