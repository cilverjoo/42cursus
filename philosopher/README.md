...
* 헤더 : <sys/wait.h>
	...
	pid 매개 변수는 대기 할 하위 프로세스 세트를 지정합니다. pid가 -1이면 호출은 자식 프로세스를 기다립니다. pid가 0이면 호출은 호출자의 프로세스 그룹에서 자식 프로세스를 기다립니다. pid가 0보다 크면 호출은 프로세스 ID가 pid 인 프로세스를 기다립니다. pid가 -1보다 작으면 호출은 프로세스 그룹 ID가 pid의 절대 값과 같은 프로세스를 기다립니다.
